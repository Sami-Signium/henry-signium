export default async function handler(req, context) {
  ...
  try {
    ...
    // Dynamic from-date: today minus 7 days
    ...
    const [mgmtRes, maRes, ceeRes] = await Promise.all([
      ...
      // CEE English - corrected query
      fetch('https://newsapi.org/v2/everything?' + new URLSearchParams({
        q: '(CEO OR appointed OR acquisition OR merger) AND (Poland OR Romania OR Hungary OR Czech OR Slovakia)',
        language: 'en',
        sortBy: 'publishedAt',
        pageSize: 40,
        from: from,
